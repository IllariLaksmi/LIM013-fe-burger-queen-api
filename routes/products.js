...
module.exports = (app, nextMain) => {
  ...
  app.get('/products/:productId', requireAuth, (req, resp, next) => {
    ...
    if (!(productId) || !req.headers.authorization) {
      // dataError(!id, !req.headers.authorization, resp);
      return dataError(!productId, !req.headers.authorization, resp);
    }
    getDataByKeyword('products', '_id', productId)
      .then((result) => {
        // eslint-disable-next-line no-param-reassign
        result[0]._id = productId.toString();
        return resp.status(200).send(result[0]);
      })
      .catch(() => resp.status(404).send({ message: 'The products does not exits' }));
  });
  /**
     * @name POST /products
